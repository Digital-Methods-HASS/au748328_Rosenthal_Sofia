...
```{r task}
got_nrc_summary <- got_nrc_n5 %>%
  group_by(sentiment) %>%
  summarise(total_count = sum(n)) %>%
  arrange(desc(total_count))

#Hvilke følelser er de mest fremtrædende?


# Graf over de mest dominerende følelser
got_nrc_summary_gg <- ggplot(got_nrc_summary, aes(x = reorder(sentiment, total_count), y = total_count, fill = sentiment)) +
  geom_col(show.legend = FALSE) +
  coord_flip() +
  theme_minimal() +
  labs(x = "Sentiment", y = "Total Count", title = "Most Dominant Sentiments in Game of Thrones") +
  theme(
...
# Gem grafen
ggsave(plot = got_nrc_summary_gg, 
       here("figures", "got_nrc_summary.png"), 
       height = 6, 
       width = 8)

#Mest fremtrædende ord indenfor hver følelse:
  
got_nrc_gg <- ggplot(data = got_nrc_n5, aes(x = reorder(word, n), y = n, fill = sentiment)) +
  geom_col(show.legend = FALSE) + # Kolonnegraf uden forklaring på farver
  facet_wrap(~sentiment, ncol = 2, scales = "free") + # Opdel efter følelse
  coord_flip() + # Vend akserne, så teksten bliver lettere at læse
  theme_minimal() +
  theme(
    axis.text.y = element_text(size = 10), # Juster tekststørrelse
    strip.text = element_text(size = 12, face = "bold") # Gør overskrifterne tydelige
  ) +
  labs(
    x = "Word",
    y = "Count",
    title = "Top 5 Words for Each Sentiment in Game of Thrones"
  )
...
# Gem grafen
ggsave(plot = got_nrc_gg, 
       here("figures", "got_nrc_top_words.png"), 
       height = 8, 
       width = 10)
...
Man kan se at de mest fremtrædende følelser i Game of Thrones er tillid, positivitet,  negativitet. At tillid er en af den mest fremtrædende følelse er interessant og også meget forventet, samtidig med at der er mange negativt ladede ord. Hvis man kender serien ved man at der sker rigtig mange negative ting. 

De mest fremtrædende ord indenfor hver følelse er
- Tillid: Lord, father, brother, Stark, horse
- Positivitet: Lord, king, brother, found, knight
- Negativitet: Lord, Stark, stone, word, cold

Noget interessant er at "lord" indgår som alle følelser. Dette giver god mening, alt efter hvilken "lord" der bliver omtalt om, hvem der omtaler dem, må følelsen svinge. "Brother" og "father" associeres med tillid og positivitet, hvilket antyder vigtigheden af familie/hus i serien. "Knight" er et positivt ord, hvilket passer til series temaer. At "Stark" både er tillid og negativitet viser tydeigt, hvordan synet på familien Stark er forskelligt alt efter, hvem der omtaler dem. At "king" er positivt viser hele problematikken med, hvem der skal være konge, og hvordan der stræbes efter denne titel. 
